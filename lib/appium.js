...
var loggerjs = require('./server/logger.js')
  ...
  , SelendroidBrowser = require('./devices/android/selendroid-browser.js')
...
var DT_IOS = "ios"
  ...
  , DT_SELENDROID_WEB = "selendroidweb"
...
Appium.prototype.getDeviceFromMJSONWP = function (args, caps) {
  ...
  var type =  this.getDeviceTypeFromAutomationName(automation, browser) ||
              this.getDeviceTypeFromBrowserName(browser, automation) ||
...
Appium.prototype.getDeviceTypeFromAutomationName = function (automation, browser) {
  if (automation.indexOf('selendroid') !== -1) {
    return browser ? DT_SELENDROID_WEB : DT_SELENDROID;
  }
};

Appium.prototype.getDeviceTypeFromBrowserName = function (browser, automation) {
  if (browser === "safari") {
    return DT_SAFARI;
  } else if (browser === "browser") {
    return automation === "selendroid" ? DT_SELENDROID_WEB : DT_CHROME;
  } else if (_.contains(["chrome", "chromium", "chromebeta"], browser)) {
    return DT_CHROME;
  }
};
...
Appium.prototype.getNewDevice = function (deviceType) {
  var DeviceClass = (function () {
    switch (deviceType) {
      ...
      case DT_SELENDROID:
        return Selendroid;
      case DT_SELENDROID_WEB:
        return SelendroidBrowser;
      case DT_FIREFOX_OS:
        return FirefoxOs;
      default:
