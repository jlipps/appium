...
class DriverCommand {
    ...
    await spinWith(this.isJsonOutput, lsMsg, async () => {
      ...
      for (const [driver, data] of _.toPairs(drivers)) {
        ...
        if (installType !== INSTALL_TYPE_NPM) {
          // don't need to check for updates on non-npm drivers
          continue;
        }
  ...
  async update ({driver, unsafe}) {
      ...
      try {
        await spinWith(this.isJsonOutput, `Checking if driver '${d}' is updatable`, () => {
          if (this.config.installedDrivers[d].installType !== INSTALL_TYPE_NPM) {
            throw new Error(`Driver '${d}' was not installed via npm, so we can't check for updates`);
          }
        });
        const update = await spinWith(this.isJsonOutput, `Checking if driver '${d}' needs an update`, async () => {
          ...
          if (!(update.safeUpdate || update.unsafeUpdate)) {
            throw new Error(`Driver '${d}' had no updates available`);
          }
          return update;
        });
        ...
        await spinWith(this.isJsonOutput, `Updating driver '${d}' from ${update.current} to ${updateVer}`, async () => {
          await this.updateDriver(d, updateVer);
        });
    ...
    for (const [d, err] of _.toPairs(errors)) {
      if (/no updates available/.test(err) || /not installed via npm/.test(err)) {
        log(this.isJsonOutput, `- ${err.message}`.yellow);
      } else {
