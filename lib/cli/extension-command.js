...
export default class ExtensionCommand {
  ...
  async execute (args) {
    ...
    return await this[cmd].bind(this)(args);
  }
  ...
  async install ({ext, installType}) {
    ...
    if (installType === INSTALL_TYPE_LOCAL) {
      ...
      extData = await this.installViaNpm({ext: installSpec});
  ...
  async installViaNpm ({ext, pkgName, pkgVer}) {
    ...
    try {
      ...
      extData.installPath = installPath;
      return extData;
    } catch (err) {
